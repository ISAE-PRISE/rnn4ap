...
/**
 * @brief GRU network optimizer. Loads and normalizes IMU data as inputs from csv files
...
//	my_log_handlers[10].create_report(reportfilename.c_str());
	...
	{
		my_gru.load(net_filename, input_size, output_size);
		...
		first_epoch = my_gru.nbLines(logResult) - 2; //-2 because there are 2 lines in the header of the file
	}
	else
	{
		my_gru.init(nb_timestep,input_size,output_size);
		std::cout << "--------GRU optimized by " << optimizer << "--------" << std::endl;//print in the terminal
		logfile   << "--------GRU optimized by " << optimizer << "--------" << std::endl;//print in the 
	...
	void (gru::*backward)(std::vector<std::vector<float>>); //specific operations depending on the chosen optimizer
	if      (optimizer == "sgd")	{my_gru.set_lr_mu(0.001, 0.9);				            backward = &gru::backward_sgd;}
	else if (optimizer == "adam")	{my_gru.set_lr_b1_b2_epsilon(0.001, 0.9, 0.999, 10.0);	backward = &gru::backward_adam;}
	else if (optimizer == "adamax")	{my_gru.set_lr_b1_b2_epsilon(0.001, 0.9, 0.999, 10.0);	backward = &gru::backward_adamax;}
	...
	for(i1=first_epoch+1;i1<=epoch_max;i1++)//EPOCH
	{
		...
		for(i7 = 0; i7 < nb_logs; i7++){//TRAIN
			...
			{
				my_gru.clear_momentums();
				auto start_bp = std::chrono::high_resolution_clock::now();
				my_gru.forward(train_input[i2]);
				(my_gru.*backward)(train_target[i2]);
		...
		for(i6=0; i6<nb_logs; i6++) //TEST
		{
			...
			for (i2=0;i2<max_samples;i2++)
			{
				...
				my_gru.get_error(test_target[i2]);
				test_loss += my_gru.get_mse();
				nb_test_batches++;
			}
		}
